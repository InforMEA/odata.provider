...
-- informea_documents
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents` AS
  SELECT
    1 schemaversion,
    a.uuid AS `id`,
    dp.field_date_value AS published,
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_types` AS
  SELECT
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_authors` AS
  SELECT
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_keywords` AS
  SELECT
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_title` AS
  SELECT
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_description` AS
  SELECT
...
-- @todo
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_identifiers` AS
  SELECT
    NULL `id`,
    NULL document_id,
    NULL `name`,
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_files` AS
  SELECT
    CONCAT(a.nid, '-', d.langcode) AS id,
    a.id AS document_id,
...
-- @todo:
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_tags` AS
  SELECT
    NULL `id`,
    NULL `document_id`,
    NULL `language`,
...
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_references` AS
  SELECT
      NULL AS `id`,
      NULL AS document_id,
      NULL AS type,
...
-- @todo:
CREATE OR REPLACE SQL SECURITY DEFINER VIEW `informea_documents_treaties` AS
  SELECT
